...
const principles: Principle[] = [
  ...
  {
    group: 'Explanation & Interpretability',
    name: 'Source Citation for Grounded Responses',
    description:
      'Every assertion or fact generated by the LLM that relies on external data (Retrieval-Augmented Generation/RAG) must be accompanied by a visible, clickable citation or reference to the specific source document or data segment it was derived from. This enables users to audit the factual basis of the output and prevents over-reliance on non-grounded (hallucinated) claims.',
    example:
      'An LLM response stating, "The project budget increased by 15% in Q3" allows the user to click the reference to view the line item in the source spreadsheet or document.',
  },
  {
    group: 'Explanation & Interpretability',
  ...
  {
    group: 'Graceful Degradation & Error Handling',
    name: 'Input Data Completeness Signaling',
    description:
      'When a user asks a question and the LLM determines the available contextual data is missing critical information or is insufficient to generate a complete answer, the system should explicitly signal the data gap and suggest options to proceed. This actively manages user expectations and avoids silent failures.',
    example:
      'A user asks to summarize a meeting, and the AI responds, "I can summarize the first hour based on the transcript, but the last 15 minutes of the recording were corrupted. Would you like me to proceed with the available data or check the calendar for related follow-up notes?"',
  },
  {
    group: 'Graceful Degradation & Error Handling',
  ...
  {
    group: 'Data',
    name: 'Data Access Minimization',
    description:
      'AI components, models, and agents should be architected to operate using only the minimal set of data required for the immediate task, enforcing the principle of least-privilege access at the infrastructure level. This is a technical guardrail to prevent unauthorized data leakage or exploitation of adjacent data sources.',
    example:
      'An AI agent that analyzes customer sentiment from incoming messages is granted only read-only access to the messages necessary for the analysis, and is explicitly blocked from accessing PII, billing systems, or other customer data sources unless those permissions are separately and explicitly granted for a specific task.',
  },
  {
    group: 'Data',
    name: 'Contextual Data Scope Review',
    description:
      'The user must be able to see and control the specific set of documents, emails, or data sources the LLM is accessing and considering for the current query (its "context window"). This makes the boundaries of the system\'s knowledge transparent and allows the user to correct the scope, preventing accidental inclusion or exclusion of sensitive or necessary data.',
    example:
      'Before a summarization runs, the interface displays a list of aggregated documents ("5 emails, 1 budget spreadsheet, this AI Design Principles document") and lets the user quickly uncheck a source or add a new one.',
  },
  {
    group: 'Data',
...
> = {
  'Explanation & Interpretability': {
    icons: [Eye, BarChart2, GitBranch, MessageSquare, Gauge, BookOpen, Share2, BookMarked],
    color: '#8B5CF6',
  },
  'Graceful Degradation & Error Handling': {
    icons: [ShieldAlert, RefreshCw, Radar, AlertTriangle, LifeBuoy, GitMerge, BellRing, TrendingUp, FlaskConical, FileQuestion],
    color: '#F97316',
  },
  ...
  Data: {
    icons: [ShieldOff, Filter, Database],
    color: '#06B6D4',
  },
}
...
export function PrinciplesView() {
  ...
  const filteredPrinciples = lq
    ? principles.filter(
        (p) =>
          p.name.toLowerCase().includes(lq) ||
          p.description.toLowerCase().includes(lq) ||
          p.example.toLowerCase().includes(lq) ||
          p.group.toLowerCase().includes(lq),
      )
    : principles

  const filteredGrouped = filteredPrinciples.reduce(
    (acc, p) => {
  ...
  // Typeahead: up to 8 suggestions matched by name first, then others
  const suggestions: SearchSuggestion[] = lq
    ? principles
        .filter((p) => p.name.toLowerCase().includes(lq))
        .slice(0, 8)
        .map((p) => ({ name: p.name, group: p.group, groupColor: groupConfig[p.group].color }))
    : []

  const totalMatches = filteredPrinciples.length

  // Scroll to a specific card, then highlight it briefly
  const handleSelectSuggestion = useCallback(
    (s: SearchSuggestion) => {
      setQuery('')
      setHighlightedName(s.name)
      posthog.capture('search_suggestion_selected', { principle: s.name, group: s.group })

      // Wait a tick for the filter to clear, then scroll
      requestAnimationFrame(() => {
        const el = cardRefs.current[s.name]
        if (el) {
          const NAVBAR_HEIGHT = 56
          const top = el.getBoundingClientRect().top + window.scrollY - NAVBAR_HEIGHT - 32
        ...
        <div className="flex-1 min-w-0 px-6 py-12 lg:px-10">
          ...
          {Object.keys(filteredGrouped).length === 0 && (
            <div className="text-center py-24 text-gray-400">
              <p className="text-lg">No principles match &ldquo;{query}&rdquo;</p>
              <button
                onClick={() => setQuery('')}
          ...
          <div className="space-y-12">
            {Object.entries(filteredGrouped).map(([group, groupPrinciples]) => {
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {groupPrinciples.map((principle, index) => {
                      const allIdx = principles.findIndex((p) => p.name === principle.name)
                      const Icon = config.icons[allIdx % config.icons.length]
                      return (
                        <div
                          key={principle.name}
                          ref={(el) => { if (el) cardRefs.current[principle.name] = el }}
                        >
                          <PrincipleCard
                            name={principle.name}
                            ...
                            highlighted={highlightedName === principle.name}
                            onView={() =>
